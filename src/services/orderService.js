...
const createOrder = async (userId, addressId, paymentMethod, couponDiscount = 0, shippingCharge = 0, couponCode = null) => {
  ...
  if (totalAmount < 0) totalAmount = 0; // Ensure total doesn't go negative
   ...
   // Separate Logic for Payment Methods
...
const updateOrderStatus = async (orderId, status, paymentStatus, reason = null) => {
  ...
  if ((status === 'Cancelled' || status === 'Returned') && (Date.now() - new Date(order.orderDate).getTime() > sevenDays)) {
  ...
  if ((status === 'Cancelled' || status === 'Returned') && order.status !== 'Cancelled' && order.status !== 'Returned' && order.paymentStatus === 'Completed') {
  ...
  if (paymentStatus) {
    ...
    updateData.paymentStatus = order.paymentStatus === 'Refunded' ? 'Refunded' : (order.paymentMethod === 'COD' ? 'Failed' : order.paymentStatus);
  }
...
const updateOrderItemStatus = async (orderId, itemId, status, reason = null) => {
  ...
  // 7-day limit check 
  const sevenDays = 7 * 24 * 60 * 60 * 1000;
  if ((status === 'Cancelled' || status === 'Return Requested') && (Date.now() - new Date(order.orderDate).getTime() > sevenDays)) {
  ...
  if ((status === 'Cancelled' || status === 'Returned') && (oldStatus !== 'Cancelled' && oldStatus !== 'Returned')) {
    ...
    const isDamaged = (reason === 'Damaged or leaking bottle' || item.reason === 'Damaged or leaking bottle');
  ...
  if ((status === 'Cancelled' || status === 'Returned') && order.paymentStatus === 'Completed') {
    const orderSubtotal = order.items.reduce((acc, i) => acc + (i.price * i.quantity), 0);
    const itemTotal = item.price * item.quantity;
    const itemCouponDiscount = orderSubtotal > 0 ? Math.round((itemTotal / orderSubtotal) * (order.couponDiscount || 0)) : 0;
