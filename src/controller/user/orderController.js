...
const downloadInvoice = async (req, res) => {
  try {
    ...
    doc.fontSize(10).font('Helvetica')
      ...
      .text('Phone: +91 98765 43210 | Email: support@junoonperfumes.com', 50, 100);

    doc.fontSize(16).font('Helvetica-Bold').text('INVOICE', 400, 45, { align: 'right' });
    doc.fontSize(10).font('Helvetica')
    ...
    if (address) {
      ...
      doc.fontSize(10).font('Helvetica')
        ...
        .text(`${address.city}, ${address.state} - ${address.pincode}`, 300, 195)
    ...
    const itemCouponDiscount= orderSubtotal > 0 ? Math.round((subtotal / orderSubtotal) * (order.couponDiscount || 0)) : 0;
    ...
    if (order.couponDiscount > 0) {
      ...
      doc.text(`- INR ${itemCouponDiscount.toLocaleString()}`, 480, summaryY + 45, { width: 70, align: 'right' });
    }

    doc.font('Helvetica-Bold').fontSize(12)
      .text('Total Paid:', 350, summaryY + 70, { width: 100, align: 'right' });

    doc.text(`INR ${finalItemAmount.toLocaleString()}`, 480, summaryY + 70, { width: 70, align: 'right' });
...
module.exports = {
  loadWallet,downloadInvoice,cancelOrderItem,returnOrderItem,placeOrder,applyCouponAjax,loadOrderSuccess,loadMyOrders,loadOrderDetails
