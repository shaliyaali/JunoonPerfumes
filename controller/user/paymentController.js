...
const createRazorpayOrder= async (req,res)=>{
  try{
    ...
    const newOrder = await orderService.createOrder(userId, addressId, 'Razorpay', couponDiscount, 0, couponCode);
    ...
    console.error(error);
    res.status(500).json({success: false, message:'could not initialize Razorpay order'}) //
  }
}
...
const verifyRazorpayPayment= async (req,res)=>{
  try{
    ...
    if(generatedSignature === razorpay_signature){ 
      ...
      if (!order) {
          return res.json({ success: false, message: 'Verification Error: Order not found in database' });
      }
      ...
      res.json({ success: false, message: 'Payment verification failed', orderId: order?.orderId });
    }
} catch (error) {
    console.error(error);
